...
interface IngredientFormProps {
  initialData: Partial<IngredientFormData> | null;
...
const IngredientForm: React.FC<IngredientFormProps> = ({
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Falha ao salvar a ingrediente.';
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <input type="text" name="ingredient" id="ingredient" value={formData.name} onChange={handleChange} required className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
      </div>
